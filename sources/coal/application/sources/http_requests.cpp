...
static cobalt::promise<Result<ssl_socket_type>> Connect(ServerEndpoint server_endpoint,
  ...
  const auto [connect_err] = co_await sock.next_layer()
      .async_connect(*endpoints.begin(), nothrow_use_op);
  if (connect_err) {
...
static void LogResponse(const 
  ...
  spdlog::info("Got http response {} reason: {} body size {} from {}",
...
static cobalt::promise<Result<beast::tcp_stream>>
  ...
  spdlog::info("Connecting");
  const auto [connect_err, _] = co_await stream.async_connect(
    endpoints, nothrow_use_op);
  if (connect_err) {
